...
class XenForo_BbCode_Formatter_Wysiwyg extends XenForo_BbCode_Formatter_Base
{
	...
	protected $_blockTagsEnd = '</p>|</div>|</blockquote>|</ul>|</ol>';
	...
	public function filterFinalOutput($output)
	{
		...
		$blockTagsEnd = $this->_blockTagsEnd;
		...
		$output = $this->_fixListStyles($output);
	...
	protected function _filterTagContents($open, $inner, $close)
	{
		...
		{
			$inner = preg_replace(
				'#<break-start />(?>\s*)(?!' . $this->_blockTags . '|' . $this->_blockTagsEnd . '|$)#i',
				"$close\\0$open",
				$inner
			);
			$inner = preg_replace(
				'#<break />(?>\s*)(?!' . $this->_blockTags . ')#i',
				"$close\\0$open",
				$inner
			);
		}
	...
	{
		$isOldIE = false;
		if (!empty($_SERVER['HTTP_USER_AGENT']) && preg_match('/MSIE (\d+)/', $_SERVER['HTTP_USER_AGENT'], $ieMatch))
		{
			$isOldIE = (intval($ieMatch[1]) <= 10);
		}

		$emptyParaText = ($isOldIE ? '&nbsp;' : '<br />');
	...
	protected function _fixListStyles($output)
	{
		do
		{
			$original = $output;
			$output = preg_replace_callback(
				'#<p([^>]*)>\s*<(ul|ol)([^>]*)>(.*)</\\2>\s*</p>#siU',
				array($this, '_fixListStylesCallback'),
				$output
			);
		}
	...
	protected function _fixListStylesCallback(array $match)
	{
		$pAttrs = $match[1];
		$listTag = $match[2];
		$listAttrs = $match[3];
