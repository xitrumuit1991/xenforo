...
class XenForo_Helper_String
{
	...
	protected static $_htmlReplacementPlaceholders = array();
	...
	public static function moveHtmlToPlaceholders($string)
	{
		self::$_htmlReplacementPlaceholders = array();

		return preg_replace_callback(
			'#<[^>]*>#si',
			array('XenForo_Helper_String', 'htmlPlaceHolderHandlerInternal'),
			$string
		);
	}

	public static function htmlPlaceHolderHandlerInternal(array $match)
	{
		$placeholder = "\x1A" . count(self::$_htmlReplacementPlaceholders) . "\x1A";
		self::$_htmlReplacementPlaceholders[$placeholder] = $match[0];

		return $placeholder;
	}

	public static function restoreHtmlPlaceholders($string)
	{
		$result = strtr($string, self::$_htmlReplacementPlaceholders);
		self::$_htmlReplacementPlaceholders = array();

		return $result;
	}

	public static function removeHtmlPlaceholders($string)
	{
	...
	protected static function _linkTaggedPlainTextCallback(array $match)
	{
		...
		$username = self::removeHtmlPlaceholders($match[3]);
	...
	{
		$url = self::removeHtmlPlaceholders($match[0]);
		$link = self::prepareAutoLinkedUrl($url);
