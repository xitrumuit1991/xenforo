...
abstract class XenForo_ViewPublic_Helper_Editor
{
	...
	public static function supportsRichTextEditor()
	{
		...
		if (!empty($_SERVER['HTTP_USER_AGENT']))
		{
			...
			{
				if (preg_match('#FxiOS/(\d+).(\d+)#', $_SERVER['HTTP_USER_AGENT'], $match))
				{
					// Firefox on iOS prior to version 4.0 does not fire a load event on an iframe with an empty src attribute
					// https://bugzilla.mozilla.org/show_bug.cgi?id=1263584
					$showWysiwyg = intval($match[1]) >= 4;
				}
				else if (preg_match('#OS (\d+)_(\d+)#', $_SERVER['HTTP_USER_AGENT'], $match))
				{
			...
			{
				// FF for Android has issues creating line breaks
				//$showWysiwyg = false;
			}
			else if (
