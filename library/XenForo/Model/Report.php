...
class XenForo_Model_Report extends XenForo_Model
{
	 ...
	 *
	 * @return array Format: [report comment id] => info
	 */
	...
	/**
	 * Gets a list of reporters to alert up until when the report was last opened.
	 *
	 * @param $reportId
	 *
	 * @return array Format: [user id] => user/report comment record
	 */
	public function getUsersToAlert($reportId)
	{
		$usersToAlert = array();
		foreach ($this->getReportComments($reportId, 'DESC') AS $comment)
		{
			if ($comment['is_report'])
			{
				$usersToAlert[$comment['user_id']] = $comment;
			}
			if ($comment['state_change'] == 'open')
			{
				break;
			}
		}
	...
	public function sendAlertsOnReportResolution(array $report, $comment = '')
	{
		...
		$users = $this->getUsersToAlert($report['report_id']);
